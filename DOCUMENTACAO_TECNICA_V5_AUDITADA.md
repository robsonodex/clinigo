...
- [2026-09-03] Visual Enterprise -> Design System -> tailwind.config.ts & app/globals.css -> Padroniza��o global de raios em 2px austero (rounded-xs), remo��o de gradientes decorativos de IA e preserva��o rigorosa de 100% das cores do sistema.
- [2026-09-03] Componentes UI -> components/ui/button.tsx -> Trava de geometria base com rounded-xs, h-9 e remo��o de sombras flutuantes.
- [2026-09-03] M�dulos -> Terapia / NPS / Documentos / Prontu�rios -> Padroniza��o dos bot�es de a��o (Excel, Registrar NPS, Novo Upload, Novo Prontu�rio).
- [2026-09-03] M�dulos -> Terapia -> Planos Terap�uticos -> app/dashboard/(clinic)/planos-terapeuticos/page.tsx -> Ajuste de espa�amento responsivo do cabe�alho e abas evitando que o bot�o Novo Plano vaze da tela.
- [2026-09-03] Financeiro -> Repasse / Contratos -> app/dashboard/(clinic)/financial/payroll/contracts/page.tsx -> Prote��o defensiva para Array em doctors e contracts evitando TypeError (doctors.map is not a function).
...
### [03/09/2026] Recepção → Painel de TV: Restauração do Botão 'Chamar' por Padrão & Isolamento Orientado a Dados
- **Módulo**: Recepção (`app/dashboard/(clinic)/recepcao/page.tsx`) & Painel de TV
...
- **Alteração Cirúrgica**:
  ...
  2. Botão 'Chamar' restaurado na coluna 'Aguardando' da Recepção como padrão ativo do sistema para todas as clínicas (incluindo Worldsensory e novas clínicas).
  3. Exceção da clínica 'Espaço Incluir' isolada no banco de dados com valor `FALSE` e justificativa histórica registrada em migration/seed.
  4. Removida qualquer dependência de nome ou UUID hardcoded no código frontend/APIs, em cumprimento rigoroso à Regra 5.2.1 de Segurança e LGPD.
  5. Adicionado toggle de 'Chamada no Painel de TV' na gestão de clínicas do Super Admin.
