...
## 🌐 Deploy para Produção (Repositório / Projeto Oficial: `clinigo-saas`)

> ⚠️ **IMPORTANTE**: O projeto de produção oficial na Vercel é o **`clinigo-saas`** (conectado à branch `master`). Todo deploy e integração de produção deve ter como alvo este projeto (**`clinigo-saas`**).
...
```bash
# Vincular/garantir vínculo ao projeto oficial clinigo-saas
