...
export default function BookingPageClient({ clinicSlugProp }: BookingPageClientProps) {
    const router = useRouter()
    const [selectedSpecialty, setSelectedSpecialty] = useState<string | undefined>()
    const [searchTerm, setSearchTerm] = useState('')

    const { data: clinic } = useQuery({
        queryKey: ['clinic', clinicSlugProp],
        queryFn: () => fetchClinicBySlug(clinicSlugProp),
        enabled: !!clinicSlugProp,
    })

    const { data: doctors, isLoading } = useQuery({
        queryKey: ['public-doctors', clinicSlugProp, selectedSpecialty],
        queryFn: () =>
            api.get<Doctor[]>('/doctors', {
                clinic_slug: clinicSlugProp,
    ...
    const { data: reviewData } = useQuery({
        queryKey: ['clinic-reviews', clinicSlugProp],
        queryFn: async () => {
            const res = await fetch(`/api/public/clinic-reviews/${clinicSlugProp}`)
            if (!res.ok) return null
            return res.json() as Promise<ClinicReviewData>
        },
        enabled: !!clinicSlugProp,
    })
                ...
                {reviewData && reviewData.total_reviews > 0 && (
                    <div className="flex items-center gap-2 mb-4 pb-4 border-b">
                        ...
                        <span className="text-sm font-semibold text-gray-800">
                            {reviewData.average_score.toFixed(1)}
                        </span>
                        <span className="text-xs text-gray-400">
                            · {reviewData.total_reviews} avaliação{reviewData.total_reviews !== 1 ? 'ões' : ''}
                        </span>
                    </div>
                )}
                ...
                {!isLoading && (!filteredDoctors || filteredDoctors.length === 0) && (
                    <div className="py-12 text-center">
                        ...
                        <button
                            ...
                            className="text-xs text-gray-400 mt-2 hover:text-black min-h-[44px]"
                        >
                            Limpar filtros
                        </button>
                    </div>
                ...
                <div className="mt-3 flex flex-wrap gap-1">
                    {isLoading ? (
                        ...
                        <>
                            {availability.days.slice(0, 3).map((day: any) => {
                                ...
                                return displaySlots.map((slot: any) => (
                                    <button
                                        ...
                                        className="px-2 py-1 text-xs border hover:border-black min-h-[44px]"
                                    >
                            ...
                            <button
                                onClick={goToProfile}
                                className="px-2 py-1 text-xs text-gray-400 hover:text-black min-h-[44px]"
                            >
                                Ver mais
                            </button>
                        </>
                        ...
                        <button
                            onClick={goToProfile}
                            className="text-xs text-gray-400 hover:text-black min-h-[44px]"
                        >
                            Ver calendario
                        </button>
                    )}
