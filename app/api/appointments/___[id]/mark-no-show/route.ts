...
import { handleApiError, BadRequestError, NotFoundError, ForbiddenError } from '@/lib/utils/errors'
...
export async function POST(
    ...
    try {
        ...
        // Only RECEPTIONIST can mark no-show
        if (currentUser.role !== 'RECEPTIONIST' && currentUser.role !== 'CLINIC_ADMIN') {
            throw new ForbiddenError('Apenas recepcionistas podem marcar faltas')
        }
      ...
      `)
        ...
        if (appointment.clinic_id !== currentUser.clinic_id) {
            throw new ForbiddenError('Agendamento não pertence à sua clínica')
        }

        // 4. Validate appointment can be marked as no-show
        if (appointment.status === 'NO_SHOW') {
            throw new BadRequestError('Este agendamento já foi marcado como falta')
        }

        if (!['SCHEDULED', 'CONFIRMED'].includes(appointment.status)) {
            throw new BadRequestError('Apenas agendamentos confirmados podem ser marcados como falta')
        }
        ...
        if (timeDiff < toleranceMs) {
            ...
            throw new BadRequestError(
                `Este agendamento ainda não passou do horário esperado. Aguarde mais ${minutesRemaining} minutos (tolerância: ${tolerance}min)`
            )
        }
        ...
        if (updateError) {
            ...
            throw new BadRequestError('Erro ao marcar falta')
        }
        ...
        const { data: rescheduleToken, error: tokenError } = await supabase
