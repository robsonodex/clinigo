...
export default function FilaEsperaPage() {
    ...
    const [form, setForm] = useState({ patient_name: '', patient_phone: '', patient_email: '', therapy_type: '', modality: 'individual', preferred_shift: 'any', urgency: 'normal', notes: '' })
    ...
    const handleSave = async () => {
        try {
            const res = await fetch('/api/waiting-list', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    <Button onClick={() => { setEditingItem(null); setForm({ patient_name: '', patient_phone: '', patient_email: '', therapy_type: '', modality: 'individual', preferred_shift: 'any', urgency: 'normal', notes: '' }); setDialogOpen(true) }}><Plus className="h-4 w-4 mr-1" />Adicionar</Button>
                </div>
            </div>
