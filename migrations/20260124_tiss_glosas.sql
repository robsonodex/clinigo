-- Migration: TISS - Sistema de Gestão de Glosas
-- Data: 2026-01-24
-- Descrição: Cria tabelas para gerenciar glosas recebidas e contestações
...
-- GLOSAS RECEBIDAS
-- ==========================================
CREATE TABLE IF NOT EXISTS tiss_glosas (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  clinic_id UUID NOT NULL REFERENCES clinics(id) ON DELETE CASCADE,
  guide_id UUID REFERENCES tiss_guides(id) ON DELETE SET NULL, -- Guia glosada (pode ser deletada)
  batch_id UUID REFERENCES tiss_batches(id) ON DELETE SET NULL, -- Lote origem
  
  -- Dados da glosa (retorno da operadora)
  glosa_code VARCHAR(10) NOT NULL, -- Código do motivo da glosa (Tabela ANS)
  glosa_description TEXT NOT NULL, -- Descrição do motivo
  glosa_value DECIMAL(10,2) NOT NULL, -- Valor glosado (R$)
  
  -- Informações adicionais
  guide_number VARCHAR(20), -- Número da guia (para referência se guide_id for null)
  operator_note TEXT, -- Observação da operadora
  
  -- Controle de datas
  received_at DATE NOT NULL, -- Data de recebimento da glosa
...
-- ==========================================
CREATE TABLE IF NOT EXISTS tiss_glosa_contests (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  glosa_id UUID NOT NULL REFERENCES tiss_glosas(id) ON DELETE CASCADE,
  
  -- Argumentação da contestação
  contest_reason TEXT NOT NULL, -- Argumento detalhado para contestar a glosa
  contest_protocol VARCHAR(20), -- Número de protocolo da contestação (da operadora)
  
  -- Status do recurso
  contest_status VARCHAR(20) NOT NULL DEFAULT 'PENDING',
  -- Status possíveis: PENDING, SENT, IN_ANALYSIS, APPROVED, REJECTED
  
  -- Resposta da operadora
  response_text TEXT, -- Texto da resposta da operadora
  approved_value DECIMAL(10,2), -- Valor aprovado após contestação (se parcial)
  
  -- Datas de controle
  submitted_at TIMESTAMP, -- Data de envio da contestação
  responded_at TIMESTAMP, -- Data de resposta da operadora
...
-- ==========================================
CREATE TABLE IF NOT EXISTS tiss_glosa_contest_attachments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  ...
  description TEXT, -- Descrição do documento (ex: "Laudo complementar")
...
-- ==========================================
-- VIEW: Métricas de Glosas por Clínica
-- ==========================================
...
COMMENT ON VIEW vw_glosa_metrics IS 'Métricas agregadas de glosas por clínica (taxa, valor, contestações)';
